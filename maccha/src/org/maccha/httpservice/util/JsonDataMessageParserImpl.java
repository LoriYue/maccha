...
public class JsonDataMessageParserImpl
  implements IDataMessageParser
{
  ...
  public void parseRequest(InputStream _inputStream, DataMessage _request)
    throws WebServiceException
  {
    ...
    try {
      ...
      ex.printStackTrace();
      SysException.handleException("UNKNOWN", ex.getCause());
    }
  ...
  public void parseRequest(String _jsonText, DataMessage _requestMessage) throws WebServiceException
  {
    ...
    try {
      ...
      Map _dataMessageJSONObject = (Map)gson.fromJson(_jsonText, HashMap.class);
      ...
      Map _parameterMap = (Map)_dataMessageJSONObject.get("parameterSet");
      _requestMessage.setParameter(_parameterMap);

      List _dateSetJSONArray = null;
      if (_dataMessageJSONObject.containsKey("dataSet")) {
        _dateSetJSONArray = (List)_dataMessageJSONObject.get("dataSet");
      }
      ...
      if ((_dateSetJSONArray != null) && (!_dateSetJSONArray.isEmpty()))
      {
        for (int i = 0; i < _dateSetJSONArray.size(); i++)
        {
          ...
          for (int k = 0; k < _metadataJSONArray.size(); k++)
          {
            ...
            Map _metadataJSONObject = (Map)_metadataJSONArray.get(k);

            Iterator _metadataJSONObjectKeys = _metadataJSONObject.keySet().iterator();
          ...
          List jsonArrayEntity = null;
          if (_dataSetJSONObject.containsKey("entityList")) {
            jsonArrayEntity = (List)_dataSetJSONObject.get("entityList");
          }
          if (jsonArrayEntity == null) jsonArrayEntity = new ArrayList();

          for (int j = 0; j < jsonArrayEntity.size(); j++) {
            Entity entity = new Entity();
            Map jsonObjectEntity = (Map)jsonArrayEntity.get(j);
            Iterator itrEntityKey = jsonObjectEntity.keySet().iterator();
            while (itrEntityKey.hasNext()) {
              ...
              if ("editStatus".equalsIgnoreCase(strName)) entity.setEditState(strValue);
