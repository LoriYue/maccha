...
public class ZipClobStringType extends AbstractLobType
{
  ...
  protected ZipClobStringType(LobHandler lobHandler, TransactionManager jtaTransactionManager)
  {
  ...
  protected Object nullSafeGetInternal(ResultSet rs, String[] names, Object owner, LobHandler lobHandler) throws SQLException
  {
    ...
    try {
      if (Base64.isArrayByteBase64(_clobStr.getBytes()))
        return deCompress(_clobStr);
    }
    catch (Exception ex) {
      ex.printStackTrace();
    }
  ...
  protected void nullSafeSetInternal(PreparedStatement ps, int index, Object value, LobCreator lobCreator) throws SQLException
  {
    try {
      ...
      String strCompress = compress((String)value);
  ...
  private String compress(String s) {
    ...
    try {
      ...
      s1 = new String(Base64.encodeBase64(bytearrayoutputstream.toByteArray()));
    }
  ...
  private String deCompress(String s) {
    String s1 = null;
    try {
      byte[] abyte0 = Base64.decodeBase64(s.getBytes());
