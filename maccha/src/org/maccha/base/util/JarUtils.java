...
{
  public static List<File> getJarFileList(Class _locaClazz, String _jarFileNameRegex)
    throws Exception
  {
    URL _url = _locaClazz.getProtectionDomain().getCodeSource().getLocation();

    List _jarFileList = new ArrayList();

    String _filePath = URLDecoder.decode(_url.getPath(), "UTF-4");
    ...
    if ((_jarDir != null) && (_jarDir.isDirectory()))
    {
      ...
      for (int i = 0; (_jarFiles != null) && (i < _jarFiles.length); i++)
      {
        ...
        if ((!StringUtils.isNotNull(_jarFileNameRegex)) || (_jarFileLastName.indexOf(_jarFileNameRegex) >= 0))
  ...
  public static List<String> getJarResoPathList(File _jarFile, String _resoFileNameRegex)
    throws Exception
  {
    List _resoPathList = new ArrayList();
    ZipFile _zipFile = null;
    try {
      _zipFile = new ZipFile(_jarFile);
      _enum = _zipFile.entries();
      ...
      for (int i = 0; (_enum != null) && (_enum.hasMoreElements()); )
      {
        ...
        if ((StringUtils.isNotNull(_resoFileNameRegex)) && (_zipEntryLastName.indexOf(_resoFileNameRegex) < 0))
          continue;
